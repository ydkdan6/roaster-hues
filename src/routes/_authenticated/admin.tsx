...
interface Profile { id: string; full_name: string; email: string; auth_id_code: string; department?: string | null; }
...
function Admin() {
  ...
  async function load() {
    const [l, p, d, r] = await Promise.all([
      ...
      supabase.from("profiles").select("id, full_name, email, auth_id_code, department"),
    ...
    if (authIdRequests.data) setAuthIdRequests(authIdRequests.data as AuthIdRequest[]);
  }
        ...
        <Tabs defaultValue="leaves">
          ...
          <TabsContent value="reports" className="mt-4">
           ...
           <TabsContent value="users" className="mt-4 space-y-6">
             <Card>
               <CardHeader><CardTitle className="flex items-center gap-2"><Inbox className="w-5 h-5 text-primary" />Auth ID recovery requests</CardTitle></CardHeader>
               <CardContent className="space-y-3">
                 {authIdRequests.filter((request) => request.status === "pending").length === 0 && <p className="text-sm text-muted-foreground">No pending recovery requests.</p>}
                 {authIdRequests.filter((request) => request.status === "pending").map((request) => {
                   const matchedProfile = profiles.find((profile) => profile.email.toLowerCase() === request.registered_email.toLowerCase());
                   return <div key={request.id} className="rounded-lg border border-border p-4 space-y-3">
                     <div className="flex items-start justify-between gap-3"><div><div className="font-medium">{request.full_name}</div><div className="text-xs text-muted-foreground">{request.registered_email}{request.department ? ` · ${request.department}` : ""}</div></div><Badge variant="secondary">pending</Badge></div>
                     <p className="text-sm text-muted-foreground">{request.message}</p>
                     <div className="flex flex-wrap items-center gap-2"><div className="rounded-md bg-secondary px-3 py-2 text-sm">{matchedProfile ? `Auth ID: ${matchedProfile.auth_id_code}` : "No matching profile found"}</div><Button size="sm" variant="outline" onClick={() => matchedProfile && copyUserId(matchedProfile.auth_id_code)} disabled={!matchedProfile}><Copy />Copy ID</Button><Button size="sm" onClick={() => resolveRequest(request.id)}>Mark resolved</Button></div>
                   </div>;
                 })}
               </CardContent>
             </Card>
                ...
                {profiles.map((p) => (
                  <div key={p.id} className="flex items-center justify-between p-3 rounded border border-border">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">{p.email}{p.department ? ` · ${p.department}` : ""}</div>
                      <div className="mt-1 flex items-center gap-2"><span className="font-mono text-xs tracking-wider text-primary">{p.auth_id_code}</span><Button type="button" size="icon" variant="ghost" className="h-7 w-7" onClick={() => copyUserId(p.auth_id_code)} aria-label={`Copy Auth ID for ${p.full_name || p.email}`}>{copiedId === p.auth_id_code ? <Check /> : <Copy />}</Button></div>
                    </div>
                    <div className="flex items-center gap-3">
